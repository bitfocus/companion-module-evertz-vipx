...
export function getActions(instance) {
	...
	const nameChoiceFrom = (arr, labelKey = 'name') => arr.map((x) => ({ id: x.name, label: `${x.name} (${x.id})` }))
	...
	return {
		...
		set_window_audio_by_ids: {
			...
			callback: async (event) => {
				...
				const window = { id: Number(event.options.window_id) }
				const audio =
					String(event.options.audio_index || '').trim() === '' ? null : { index: Number(event.options.audio_index) }
