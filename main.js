...
class EvertzVIPXInstance extends InstanceBase {
	...
	async handshake() {
		...
		const negotiated = res?.result?.server_selected_version ?? res?.server_selected_version ?? 1
	...
	async refreshAll() {
		try {
			...
			this.log(
				'debug',
				`Refreshed lists — displays=${this._displays.length}, layouts=${this._layouts.length}, snapshots=${this._snapshots.length}`
			)
		} catch (e) {
